...
def usage():
    ...
    message += " fsal <fsal name> | v3_full | v4_full | rpc |"
    message += " auth] \n"
    ...
    message += "v3_full | v4_full | rpc | auth] \n"
...
commands = ('help', 'list_clients', 'deleg', 'global', 'inode', 'iov3', 'iov4',
	    ...
	    'disable', 'status', 'pool', 'v3_full', 'v4_full', 'auth')
...
# requires fsal name
    ...
    if not len(sys.argv) == 3:
	print "Option \"%s\" must be followed by all/nfs/fsal/v3_full/v4_full/rpc/auth." % (command)
	usage()
    command_arg = sys.argv[2]
    if command_arg not in ('all', 'nfs', 'fsal', 'v3_full', 'v4_full', 'rpc', 'auth'):
	print "Option \"%s\" must be followed by all/nfs/fsal/v3_full/v4_full/rpc/auth." % (command)
	usage()
